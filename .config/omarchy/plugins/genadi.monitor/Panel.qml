...
Panel {
  ...
  // Monitor geometry for the ARRANGEMENT map. hyprctl carries positions,
  // scale, and transform, which omarchy-monitor-state's display list doesn't.
  property var geoDisplays: []
  property bool dragActive: false

  readonly property var arrangeRects: geoDisplays.map(logicalRect)
  readonly property var arrangeBounds: {
    if (!arrangeRects.length) return { x: 0, y: 0, w: 1, h: 1 }
    var minX = Math.min.apply(null, arrangeRects.map(function(r) { return r.x }))
    var minY = Math.min.apply(null, arrangeRects.map(function(r) { return r.y }))
    var maxX = Math.max.apply(null, arrangeRects.map(function(r) { return r.x + r.w }))
    var maxY = Math.max.apply(null, arrangeRects.map(function(r) { return r.y + r.h }))
  ...
  function setScale(scale) {
    actionProc.command = ["bash", "-c", "gazarchy-cmd-monitor-scale " + scale]
    if (!actionProc.running) actionProc.running = true
  }

  // ---- Arrangement (logical-pixel monitor map) ----
  function logicalRect(d) {
    var scale = Number(d.scale) || 1
    var swap = (Number(d.transform) || 0) % 2 === 1
    return {
      name: d.name,
      focused: d.focused === true,
      x: Number(d.x) || 0,
      y: Number(d.y) || 0,
      w: (swap ? Number(d.height) : Number(d.width)) / scale,
      h: (swap ? Number(d.width) : Number(d.height)) / scale
    }
  }

  function snapAxis(value, edges, threshold) {
    var best = value
  ...
  function commitArrangement(name, lx, ly, threshold) {
    var moved = arrangeRects.find(function(r) { return r.name === name })
    if (!moved || (Math.abs(lx - moved.x) < 1 && Math.abs(ly - moved.y) < 1)) return

    // Snap the dragged monitor to its neighbors' edges: adjacency and alignment.
    var xE = []
    var yE = []
    arrangeRects.forEach(function(o) {
      if (o.name === name) return
      xE.push(o.x - moved.w, o.x + o.w, o.x, o.x + o.w - moved.w)
      yE.push(o.y - moved.h, o.y + o.h, o.y, o.y + o.h - moved.h)
    })
    lx = snapAxis(lx, xE, threshold)
    ly = snapAxis(ly, yE, threshold)

    // Re-anchor the layout at 0x0 and hand every monitor's position to the CLI.
    var placed = arrangeRects.map(function(r) { return r.name === name ? { name: name, x: lx, y: ly } : r })
    var minX = Math.min.apply(null, placed.map(function(p) { return p.x }))
    var minY = Math.min.apply(null, placed.map(function(p) { return p.y }))
    arrangeProc.command = ["gazarchy-cmd-monitor-arrange"].concat(
      placed.map(function(p) { return p.name + "@" + Math.round(p.x - minX) + "x" + Math.round(p.y - minY) }))
    if (!arrangeProc.running) arrangeProc.running = true
  }
  ...
  Process {
    ...
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: {
        // Never yank a monitor out from under a drag in progress.
        if (root.dragActive) return
        try { root.geoDisplays = JSON.parse(String(text || "[]")) } catch (e) {}
      }
    }
  }
    ...
    PanelKeyCatcher {
      ...
      ScrollView {
        ...
        Column {
          ...
          // ---------- Arrangement ----------
          PanelSeparator {
            visible: root.geoDisplays.length > 1
            foreground: root.bar.foreground
          }
          ...
          Column {
            ...
            Item {
              width: parent.width
              implicitHeight: arrangeHeader.implicitHeight
            ...
            Item {
              ...
              readonly property var bounds: root.arrangeBounds
              readonly property real fit: Math.min(
                (width - Style.space(16)) / bounds.w,
                (height - Style.space(16)) / bounds.h)
              readonly property real offX: (width - bounds.w * fit) / 2
              readonly property real offY: (height - bounds.h * fit) / 2
  ...
  component ArrangeBox: Rectangle {
    id: box
    required property var rect

    x: arrangeCanvas.offX + (rect.x - arrangeCanvas.bounds.x) * arrangeCanvas.fit
    y: arrangeCanvas.offY + (rect.y - arrangeCanvas.bounds.y) * arrangeCanvas.fit
    width: Math.max(Style.space(24), rect.w * arrangeCanvas.fit)
    height: Math.max(Style.space(16), rect.h * arrangeCanvas.fit)

    radius: Style.space(4)
    color: rect.focused
      ? Style.selectedFillFor(root.bar.foreground, Color.accent)
      : Style.hoverFillFor(root.bar.foreground, Color.accent)
    border.color: boxDrag.drag.active ? Color.accent : root.bar.foreground
    border.width: boxDrag.drag.active ? 2 : 1

    Text {
      anchors.centerIn: parent
      width: parent.width - Style.space(8)
      text: box.rect.name + (box.height > Style.space(34)
    ...
    MouseArea {
      ...
      drag.target: box
      drag.minimumX: 0
      drag.maximumX: arrangeCanvas.width - box.width
      drag.minimumY: 0
      drag.maximumY: arrangeCanvas.height - box.height

      onPressed: root.dragActive = true
      onReleased: {
        root.dragActive = false
        root.commitArrangement(box.rect.name,
          (box.x - arrangeCanvas.offX) / arrangeCanvas.fit + arrangeCanvas.bounds.x,
          (box.y - arrangeCanvas.offY) / arrangeCanvas.fit + arrangeCanvas.bounds.y,
          Style.space(14) / arrangeCanvas.fit)
      }
    }
  }
